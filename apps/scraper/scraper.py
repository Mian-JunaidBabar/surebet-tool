...
def extract_event_data(row_element, target_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"✅ Extracted: {event_name}")
...
def scrape_betting_data(target_url: str, target_name: str) -> List[Dict[str, Any]]:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            logger.info(f"🎉 Scraping completed! Successfully extracted {len(scraped_data)} events")
...
def send_data_to_backend(scraped_data: List[Dict[str, Any]]) -> bool:
    ...
    logger.info(f"📤 Sending {len(scraped_data)} events to backend...")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"📈 Events processed: {response_data.get('events_processed', 'Unknown')}")
...
def fetch_scraper_targets() -> List[Dict[str, Any]]:
    ...
    """
    logger.info(f"📥 Fetching scraper targets from backend...")
    logger.info(f"🎯 URL: {SCRAPER_TARGETS_URL}")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ Fetched {len(targets)} active scraper target(s)")
...
def main():
    ...
    try:
        ...
        if not targets:
            logger.warning("❌ No active scraper targets found. Exiting...")
            return
        ...
        for idx, target in enumerate(targets, 1):
            ...
            if not target_url:
                logger.warning(f"⚠️  Skipping target {idx}: Missing URL")
                continue
            
            logger.info(f"\n{'=' * 60}")
            logger.info(f"📍 Processing target {idx}/{len(targets)}: {target_name}")
            logger.info(f"{'=' * 60}")
            ...
            if scraped_data:
                ...
                logger.warning(f"⚠️  No data scraped from {target_name}")
